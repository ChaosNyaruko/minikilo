...
char *MK_HL_keywords[] = {
    "if",    "else", "let",   "fn",     "macro", "return", "first|",   "rest|",
    "last|", "len|", "push|", "print|", "eval|", "quote|", "unquote|", NULL};

struct editorSyntax HLDB[] = {
    {"c", C_HL_extensions, C_HL_keywords, "//", "/*", "*/",
     HL_HIGHLIGHT_NUMBERS | HL_HIGHLIGHT_STRINGS},
    {"monkey", MK_HL_extensions, MK_HL_keywords, "//", "/*", "*/",
     HL_HIGHLIGHT_NUMBERS | HL_HIGHLIGHT_STRINGS},
};
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    if (filerow >= E.numrows) {
      ...
      for (int j = 0; j < len; j++) {
        if (iscntrl(c[j])) {
          // TODO: esc -> 
          // but esc is used for special functionalities: cancel
          // saving/searching try F1/F2/.. stuff.
          char sym = (c[j] <= 27) ? '@' + c[j] : '?';
